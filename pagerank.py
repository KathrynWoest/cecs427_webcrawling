...
def calculate(graph, output_file):
    ...
    try:
        # calculate the initial page rank for every node, the dampening factor, and the equilibrium cutoffs
        ...
        print("Error in setting up the page rank calculations, so caluculation terminated. Error:", e)
        return
    ...
    try:
        ...
        while not equilibrium:
            ...
            for node, rank in last_step_graph.nodes(data="page_rank"):
                ...
                # otherwise, let the current node keep all of its current rank (per lecture slides)
                else:
                    newpr_graph.nodes[node]["page_rank"] = rank
            
            # to prevent rank from pooling in some nodes due to structure, restribute rank based on dampening value 's'
            for node, rank in newpr_graph.nodes(data="page_rank"):
                newpr_graph.nodes[node]["page_rank"] = (rank * s) + ((1 - s) / num_nodes)
            
            # determine if equilibrium has been reached
            max_diff = 0
            for node, rank in newpr_graph.nodes(data="page_rank"):
                diff = abs(rank - last_step_graph.nodes[node]["page_rank"])
                if diff > max_diff:
                    max_diff = diff
        ...
        print("Error in calculating the page rank, so calculation terminated. Error:", e)
        return

    # store the results in the given output file
    print("Page rank calculated successfully.")
